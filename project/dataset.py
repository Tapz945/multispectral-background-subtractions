...
def read_img(directory, in_channels=None, label=False, patch_idx=None, height=256, width=256, band_num=[]):
    ...
    # for features images
        ...
        if len(band_num)>2:
            ...
            #     new_dir = "/" + dir_list[1] + "/" + dir_list[2] + "/" + dir_list[3] + "/" + dir_list[4] + "/" + dir_list[5] + "/" + dir_list[6] + "/" + "imgMS" + "/" + img_num[0] + "_band" + str(band_num[i]) + ".jpg"
            #     X[:,:,i] = cv2.imread(new_dir)
...
def data_path_split(config):
    ...
    # config["dataset_dir"] = /mnt/hdd2/mdsamiul/multispectral/data/ groundtruth imgMS imgRGB
    for i in range(1, 6):
